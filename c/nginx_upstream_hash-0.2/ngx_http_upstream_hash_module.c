...
{
    return ngx_crc32_long(data, len) & 0x7fff;
}

static ngx_uint_t
ngx_http_upstream_hash_crc32_short(u_char *data, size_t len)
{
...
ngx_http_upstream_hash_method(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    } else if (ngx_strcmp(value[1].data, "crc32s") == 0) {
        uscf->hash_function = ngx_http_upstream_hash_crc32_short;
    } else if (ngx_strcmp(value[1].data, "5381") == 0) {
        uscf->hash_function = ngx_http_upstream_hash_5381;
